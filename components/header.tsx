...
{/* </header> */}
    ...
    <nav className="flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
  <ul className="flex w-full justify-center gap-x-5 items-center text-[0.9rem] font-medium text-gray-500 sm:w-auto sm:flex-nowrap sm:gap-x-5">
    {links.map((link) => (
      <motion.li
        className="h-3/4 flex items-center justify-center relative"
        key={link.hash}
      ...
      >
        ...
        >
          {link.name}
{/* Here we can change the nav-bar active section  */}
          {link.name === activeSection && (
            <motion.span
              className="bg-[#dbd7fb] rounded-full absolute inset-0 -z-10 dark:bg-[#9c91f4]-100"
              layoutId="activeSection"
              transition={{
                type: "spring",
                stiffness: 380,
